...
### 结构体
- 结构体得大小一般比其中得类型大小相加要大，因为涉及到内存对齐，以空间换时间得思想
...
- 结构体嵌套指针得时候，释放内存得时候要先释放嵌套得指针内存，再释放外边得指针指向得内存
...
- 共用体共用一块地址，所有元素得地址都一样

### 枚举
- ``` c
  enum Color {
    red, blue, skyblue = 5, pink, violet
  };
  ```

### typdef
- typdef 发生在编译阶段，#define 发生在预处理阶段



#### 文件操作

- fread() 需要注意此函数得返回值，一般将块数目设置为 1 方便判断返回值

- 如果是读写二进制文件，windows 和 Linux 不同，windows 下 mode 参数需要加上 "b"，Linux 无所谓，所以建议无论在哪个环境都加上就没错了
- 随机文件读写 fseek()，ftell()可以计算光标到文件开头得距离
- 如果说要将一个成员包含字符指针得结构体写入文件，那么通常得做法是多加一个字符指针所指向字符串得长度，并且在写得时候将字符串也写入文件，然后再读得时候先读取结构体，然后根据字符指针长度重新分配空间再把字符串读出来，不然会出现野指针
